...
export function createScene(container, cfg, agentNames = []) {
  ...
  // El escenario (cámara, renderer, órbita, lente, etiqueta, destello) es
  // COMPARTIDO entre mundos; aquí solo se construye el contenido del bosque.
  const stage = createStage(container, cfg)
  const { scene, camera, controls, labelEl } = stage
